...
//	private static final int NTHREAD = 32;
    ...
    public static Collection paras() {
        ...
        args.addAll(genWorkLoad1(LockFreeBSTree.class, new Object[] {}));
	...
	public void testUpdateST() {
		for (int i = 0; i < NELEMENT; ++i) {
			bst.update(bst, String.valueOf(i), "test" + i);
//			System.out.println("added " + i);
		}

		for (int i = 0; i < NELEMENT; ++i) {
			assertNotNull(bst.find(bst, String.valueOf(i)));
		}
	}
	...
	public void testRemoveST() {
		...
		for (int i = 0; i < NELEMENT; ++i) {
			bst.remove(bst, String.valueOf(i));
		}

		for (int i = 0; i < NELEMENT; ++i) {
			assertNull(bst.find(bst, String.valueOf(i)));
		}
	...
	public void testUpdateAndRemoveMT() throws Throwable {
		...
		for (int threadID = 0; threadID < NTHREAD; threadID++) {
			...
			threads[threadID] = new Thread("Thread-" + threadID) {
				...
				public void run() {
					for (int i = 0; i < NELEMENT; ++i) {
						bst.update(bst, testData[NELEMENT * id + i], "test"
                                + i);
//						System.out.println("Thread " + id + " added " + i);
		...
		for (int threadID = 0; threadID < NTHREAD; threadID++) {
			...
			threads[threadID] = new Thread("Thread-" + threadID) {
				public void run() {
					for (int i = 0; i < NELEMENT; ++i) {
						assertNotNull("cannot find " + i, bst.find(bst, testData[NELEMENT * id + i]));
//						System.out.println("Checking " + i);
		...
		for (int threadID = 0; threadID < NTHREAD; threadID++) {
			...
			threads[threadID] = new Thread("Thread-" + threadID) {
				public void run() {
					for (int i = 0; i < NELEMENT; ++i) {
						bst.remove(bst, testData[NELEMENT * id + i]);
//						System.out.println("Removed " + i);
		...
		for (int threadID = 0; threadID < NTHREAD; threadID++) {
			...
			threads[threadID] = new Thread("Thread-" + threadID) {
				public void run() {
					for (int i = 0; i < NELEMENT; ++i) {
						assertNull(bst.find(bst, testData[NELEMENT * id + i]));
//						System.out.println("Checking " + i);
					}
				}
			};
